...
@Cmd(name = "STRLEN", minArgs = 2)
public class StrLenCommand implements RedisCommand, NeedsStore {
    ...
    public byte[] execute(Connection connection, List<byte[]> argv) {
        if (argv.size() != 2) encoder.encodeError("ERR wrong number of arguments for 'STRLEN' command");

        var val = store.get(argv.get(1));
        if (val == null) return encoder.encodeInteger(0);
        if (val instanceof RedisString redisString) {
            return encoder.encodeInteger(redisString.getStringValue().length());
        }
